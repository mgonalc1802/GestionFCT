...
$(function()
{
    var crear = '<a class=" action-new btn btn-primary" href="/admin?routeName=personasContacto" data-action-name="new"><span class="action-label">Add Persona Contacto</span></a>';
                      
    //Obtiene el header de la plantilla de easyAdmin
    $(".content-header-title h1").append('Personas Contacto');
...
function traePersonasContacto()
{
    ...
    $.ajax(
    {
        ...
        success: function (response) 
        {
            ...
            var tablaPersonaContacto = $("tbody");
            ...
            for(let i = 0; i < response.length; i++)
            {
                ...
                var tdNombre = $('<td data-column="email" data-label="Nombre" class=" text- field-text" dir="ltr">\
                                    <span title="' + response[i].nombre + '">' + response[i].nombre + '</a>\
                                 </td>');
                
                var tdApellido1 = $('<td data-column="apellido1" data-label="Apellido1" class=" text- field-text" dir="ltr">\
                                        <span title="' + response[i].apellido1 + '">' + response[i].apellido1 + '</span>\
                                    </td>');

                var tdApellido2 = $('<td data-column="apellido2" data-label="Apellido2" class=" text- field-text" dir="ltr">\
                ...
                var tdAcciones = $('<td class="actions actions-as-dropdown">\
                                        <div class="dropdown dropdown-actions">\
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">\
                                                ...
                                                <a class="dropdown-item action-delete text-danger" onclick = "borrarPersonaContacto(' + response[i].id + ')" data-action-name="delete"><span class="action-label">Delete</span></a>\
                                            </div>\
                                        </div>\
                                    </td>');
                ...
                tablaPersonaContacto.append(tr);
            }       
        }
    });
}

function borrarPersonaContacto(id)
{
    $.ajax(
        {
            url: "/API/borrarPersonaContacto/" + id,
            type: 'GET',
            dataType: 'json',
            contentType: 'application/json', 
            processData: false,
